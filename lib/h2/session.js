...
class H2Session extends EventEmitter {
	constructor(transport, profile = null) {
		...
		// Keepalive PING loop. Residential proxies + NAT boxes routinely blackhole idle TCP
		// (no FIN/RST) — Node keeps thinking the socket is fine, we hand it out of the pool,
		// and the next request hangs forever waiting for a HEADERS frame from a corpse. Send a
		// PING every 30s; if no ACK within 30s, treat the connection as dead and tear it down
		// so in-flight requests fail cleanly and the pool evicts the entry.
		this._keepaliveIntervalMs = profile?.http2?.keepaliveIntervalMs ?? 30_000
		this._keepaliveTimeoutMs = profile?.http2?.keepaliveTimeoutMs ?? 30_000
		this._pingOutstanding = null
		this._keepaliveTimer = setInterval(() => this._sendKeepalivePing(), this._keepaliveIntervalMs)
	...
	_sendKeepalivePing() {
		if (this.closed || this.destroyed || this._pingOutstanding) return
		const payload = Buffer.alloc(8)
		payload.writeUInt32BE(0x68656c6c, 0)  // "hell" — magic tag identifying our probes
		payload.writeUInt32BE((Date.now() & 0xffffffff) >>> 0, 4)
		const timer = setTimeout(() => {
			// No ACK — peer is dead. Tear down; _fatalError fails in-flight streams and evicts.
			this._fatalError(Object.assign(new Error('h2 keepalive PING timed out'), { code: 'EH2KEEPALIVE' }))
		}, this._keepaliveTimeoutMs)
		timer.unref?.()
		this._pingOutstanding = { timer }
		try {
			this._sendFrame(frame.TYPE.PING, 0, 0, payload)
		} catch (e) {
			clearTimeout(timer)
			this._pingOutstanding = null
			this._fatalError(e)
	...
	_clearKeepalive() {
		...
		if (this._pingOutstanding) { clearTimeout(this._pingOutstanding.timer); this._pingOutstanding = null }
	}
		...
		switch (f.type) {
			...
			case frame.TYPE.PING:
				if (f.flags & frame.FLAG.ACK) {
					// ACK for our keepalive probe — peer is alive; clear the ACK-deadline timer.
					if (this._pingOutstanding) {
						clearTimeout(this._pingOutstanding.timer)
						this._pingOutstanding = null
					}
				} else {
	...
	request(headers, opts = {}) {
		...
		for (const [k, v] of Object.entries(headers)) {
			// Chrome (H2) sends EACH cookie as its OWN `cookie` header (HPACK indexes them
			// individually); a single combined `cookie` line is an Akamai/DataDome header-
			// ORDER fingerprint tell (HAR-verified: browser sends N cookie headers, we sent 1).
			// Split on ';' (with optional whitespace) so per-cookie header order matches Chrome.
			// Handles both string and array-valued cookie headers. H1 path unaffected.
			if (k.toLowerCase() === 'cookie') {
				const vals = Array.isArray(v) ? v : [v]
				for (const vv of vals) {
					if (typeof vv === 'string') {
						for (const c of vv.split(/;\s*/)) { if (c) list.push([k, c]) }
					} else list.push([k, vv])
				}
			}
			else if (Array.isArray(v)) for (const vv of v) list.push([k, vv])
			else list.push([k, v])
		}
