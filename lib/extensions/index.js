...
// Generic verbatim-body extension. Emits `[id-hi, id-lo, len-hi, len-lo, ...body]` — the
// standard TLS extension wire framing. Used by profiles that captured an unrecognized
// extension via fromPeet: we don't know the semantics but we know the exact bytes Chrome
// put on the wire, and for passive fingerprint-only extensions (Chrome experiments,
// telemetry, unspec'd codepoints) that's all a server observer needs. Do NOT use this for
// interactive extensions whose body must be computed per-connection (key_share, PSK,
// early_data) — those need real builders.
const CreateRawExtension = (id, body) => {
	if (!Number.isInteger(id) || id < 0 || id > 0xffff) {
		throw new Error(`CreateRawExtension: invalid extension id ${id}`)
	}
	const b = Buffer.isBuffer(body) ? body : Buffer.from(body || [])
	const hdr = Buffer.alloc(4)
	hdr.writeUInt16BE(id, 0)
	hdr.writeUInt16BE(b.length, 2)
