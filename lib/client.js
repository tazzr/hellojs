...
function performH2(conn, { method, url, host, body, userHeaders, opts, profileObj }) {
	return new Promise((resolve, reject) => {
		...
		req.on('end', () => {
			...
			// can sum from the request:firstByte + subsequent hooks.
			observability.safeEmit('request:end', { id: reqId, status, headers: resHeaders, totalBytes, durationMs: Date.now() - reqT0, responseBody: raw })
	...
	return new Promise((resolve, reject) => {
		...
		function finish(bodyEndIdx) {
			...
			const reusable = mode !== 'eof' && !wantsClose
				...
				observability.safeEmit('request:end', { id: opts._reqId, status, headers, totalBytes: raw.length, durationMs: Date.now() - h1T0, responseBody: raw })
				let bodyOut = raw
				if (opts.json && raw.length) { try { bodyOut = JSON.parse(raw.toString('utf8')) } catch {} }
