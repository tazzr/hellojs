...
class Pool {
	...
	acquire({ host, port = 443, profile, proxy, forceFresh = false, transport = 'tcp', earlyData = null, verifyTLS = true, timeouts = null, profileObj = null }) {
		...
		const p = this._acquireSlot(host, port).then(async () => {
			...
			// Slot accounting caps SIMULTANEOUS handshakes / forceFresh (Chrome-ish burst cap),
			// NOT pooled keep-alive count — otherwise routing through N different proxies to the
			// same target host (rotating residential creds) wedges the (N+1)th request for the
			// full 5-minute idle timeout. For pooled conns: release now (handshake done, in the
			// map). For forceFresh conns: hold slot until conn.close.
			let released = false
			const releaseOnce = () => { if (!released) { released = true; this._releaseSlot(host, port) } }
			if (!forceFresh) {
				this.connections.set(key, conn)
				releaseOnce()
			} else {
				const origClose = conn.close.bind(conn)
				conn.close = (reason) => { releaseOnce(); return origClose(reason) }
			}
			return conn
		})
		if (!forceFresh) {
	...
	_create(host, port, profile, proxy, key, opts = {}) {
		return new Promise((resolve, reject) => {
			...
			if (timeouts.tlsHandshake) {
				// Start the TLS-handshake stopwatch ONLY when ClientHello actually goes on the
				// wire. Previously the timer began the instant _create was called, so it counted
				// MLKEM-keypair acquisition + TCP connect + proxy CONNECT tunnel round-trip
				// before any TLS bytes shipped — giving consistent 'TLS handshake timed out'
				// errors on residential proxies that ate most of the budget in pre-TLS phases.
				// startClientHello re-fires on HRR; we only arm the timer once.
				tls.once('handshake-start', () => {
					if (settled) return
