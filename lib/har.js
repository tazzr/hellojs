...
class HarRecorder {
	...
	_makeEntry(rec, res) {
		...
		// HAR httpVersion mirrors the negotiated ALPN when we have it, else defaults to HTTP/2
		// (matches the main hot path). "HTTP/2" is what Chrome DevTools writes for h2 too.
		const httpVersion = alpnToHarVersion(res.tls?.alpn)
		...
		const entry = {
			...
			timings: {
				...
				// dns/connect/ssl aren't measured yet (would need dns/handshake events); -1
				// per HAR spec means "not applicable". Non-negative counterparts survive to
				// tools that only look at the visible fields (send/wait/receive).
				blocked: -1, dns: -1, connect: -1, ssl: -1,
			},
			// Standard HAR entry fields — servers behind CDNs vary IP per connection.
			serverIPAddress: res.tls?.serverIPAddress || '',
			connection: res.tls?.connectionId || '',
			// Chrome DevTools extension: appears in DevTools "Security" tab per request.
			...(res.tls ? { _securityDetails: harSecurityDetails(res.tls) } : {}),
			// Chrome-flavored extras that some HAR viewers surface.
			...(res.tls?.ja3 ? { _ja3: res.tls.ja3 } : {}),
			...(res.tls?.ja4 ? { _ja4: res.tls.ja4 } : {}),
			...(res.tls?.usedSession ? { _resumedSession: true } : {}),
			...(res.tls?.usedEarlyData ? { _earlyData: true } : {}),
			_hellojs: { id: rec.id },
...
// Map ALPN token → HAR httpVersion string. Chrome DevTools writes "HTTP/2" (not h2),
// "HTTP/1.1", "h3" for QUIC. Keep the same conventions so tools consuming our HAR
// interoperate with Chrome-produced ones.
function alpnToHarVersion(alpn) {
	if (!alpn) return 'HTTP/2'
	if (alpn === 'h2') return 'HTTP/2'
	if (alpn === 'http/1.1') return 'HTTP/1.1'
	if (alpn === 'h3') return 'h3'
	return alpn
}

// Build the Chrome DevTools _securityDetails extension. Fields match DevTools' own
// output so importing this HAR into a Chrome-shaped viewer shows the security tab.
// Missing fields (e.g. cert parse failure) are elided rather than nulled so viewers
// don't render empty rows.
function harSecurityDetails(tls) {
	const d = {}
	if (tls.protocol) d.protocol = tls.protocol
	if (tls.keyExchange) d.keyExchange = tls.keyExchange
	if (tls.cipher) d.cipher = tls.cipher
	// TLS 1.3 uses AEAD ciphers so there's no separate MAC; keep the field to match
	// DevTools' schema, but leave it empty for AEAD.
	d.mac = ''
	if (tls.cert) {
		if (tls.cert.subject) {
			// DevTools shows just the CN. Extract from "CN=example.com" if we can.
			const m = String(tls.cert.subject).match(/CN=([^\r\n,]+)/)
			d.subjectName = m ? m[1] : tls.cert.subject
		}
		if (tls.cert.sanList) d.sanList = tls.cert.sanList
		if (tls.cert.issuer) {
			const m = String(tls.cert.issuer).match(/CN=([^\r\n,]+)/)
			d.issuer = m ? m[1] : tls.cert.issuer
		}
		if (tls.cert.validFrom) {
			const ts = Math.floor(Date.parse(tls.cert.validFrom) / 1000)
			if (Number.isFinite(ts)) d.validFrom = ts
		}
		if (tls.cert.validTo) {
			const ts = Math.floor(Date.parse(tls.cert.validTo) / 1000)
			if (Number.isFinite(ts)) d.validTo = ts
		}
		if (tls.cert.fingerprint256) d.certificateFingerprint = tls.cert.fingerprint256
	}
	// DevTools carries a per-entry certificateId (int); we don't dedupe certs across
	// entries so just emit 0 — every viewer treats it as an opaque handle.
	d.certificateId = 0
	return d
}
