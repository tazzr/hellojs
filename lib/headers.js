// Chrome 147 request header set + the profile-driven builders that the client uses to
// assemble outgoing requests. HTTP/2 sends headers in the order keys are inserted into
// the JS object, so order is controllable from JS.
//
// POLICY: profiles are TLS-fingerprint-only. Neither this file's CHROME_147_DEFAULT_HEADERS
// nor a profile's own `.headers` block is auto-injected into outgoing requests. The wire
// contains exactly the headers the caller placed in `opts.headers` (plus body-derived
// Content-Type / Content-Length / cookie-jar Cookie that client.js still adds). Callers
// who want the classic Chrome header set should import CHROME_147_DEFAULT_HEADERS and
// spread it into their `headers` object at request time.
...
function buildH2Headers({ method, host, path, userHeaders, profile }) {
	...
	// Emit caller headers verbatim, in caller insertion order. No profile-default injection —
	// the caller is authoritative for the header set. Connection-specific headers are illegal
	// in H2 (RFC 7540 §8.1.2.2) and dropped even if the caller passes them.
...
function buildH1Headers({ host, userHeaders, profile: _profile }) {
	// Emit caller headers verbatim after Host. No profile-default injection. Host is
	// builder-managed (RFC 7230 §5.4 — exactly one Host, first) so a caller-supplied Host
	// overrides the URL-derived value rather than producing a duplicate line.
	let hostOverride
	if (userHeaders) for (const [k, v] of Object.entries(userHeaders)) {
